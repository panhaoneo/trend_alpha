...
def gen_md(ctx, path):
    ...
    if p.get("require_eps") and s.get("eps_cut"):
        L.append(f"- EPS同比门槛剔除 {s['eps_cut']} 只（无同期基数者保留并显示—）")
    L.append(f"- PDF链接: 财报=最新定期报告, 招股书=上市招股文件（来源: 巨潮资讯; — 表示暂无可用PDF）")
    L.append("")
    L.append("> 指标口径: **EPS同比** = 基本每股收益(利润表 basic_eps)同报告期同比(如中报对上年中报), — 表示无同期基数(新股/扭亏)或未披露; "
